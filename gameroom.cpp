...
// 构造函数
    ...
    connect(m_timer, &QTimer::timeout, this, &GameRoom::moveSnake);
    m_timer->start(m_speedInterval); // 每150毫秒移动一次
}

void GameRoom::setSpeedInterval(int interval)
{
    // 限制最小间隔为50ms，最大间隔为500ms
    m_speedInterval = qBound(50, interval, 500);
    m_timer->start(m_speedInterval);  // 重启定时器应用新速度
}
...
// 检查食物是否在蛇身上
bool GameRoom::isFoodOnSnake(const QPoint &point)
{
...
// 绘制蛇的函数
void GameRoom::drawSnake(QPainter &painter)
{
